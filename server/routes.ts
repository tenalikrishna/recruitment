...
export function registerRoutes(app: Express) {
  ...
  // ── Screeners list ────────────────────────────────────────────────────────────
  app.get("/api/screeners", requireAuth, requireRole("admin"), async (_req, res, next) => {
    try {
      const users = await storage.getAllAdminUsers();
      const hasRole = (role: string, ...check: string[]) =>
        role.split(",").map(r => r.trim()).some(r => check.includes(r));
      res.json(users.filter(u => hasRole(u.role, "screener")).map(({ passwordHash, ...u }) => u));
    } catch (err) { next(err); }
  });

  // ── Sync from Hostinger ───────────────────────────────────────────────────────
  app.post("/api/sync", requireAuth, requireRole("admin"), async (_req, res, next) => {
    try {
